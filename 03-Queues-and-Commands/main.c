...
int main()
{
	...
	vk_device_queue_create_info.sType =
		VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
	vk_device_queue_create_info.flags = 0;
	vk_device_queue_create_info.queueFamilyIndex = graphics_bit_index;
	vk_device_queue_create_info.queueCount = 1;
	vk_device_queue_create_info.pQueuePriorities = 0;
	...
	device_info.pQueueCreateInfos = &vk_device_queue_create_info;
	...
	vkGetDeviceQueue(device, graphics_bit_index, 0, &device_queue);

	VkCommandPoolCreateInfo vk_command_pool_create_info = { 0 };
	vk_command_pool_create_info.sType =
		VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
	vk_command_pool_create_info.pNext = NULL;
	vk_command_pool_create_info.flags =
		VK_COMMAND_POOL_CREATE_TRANSIENT_BIT |
		VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
	vk_command_pool_create_info.queueFamilyIndex = graphics_bit_index;

	VkCommandPool vk_command_pool;
	result = vkCreateCommandPool(device, &vk_command_pool_create_info,
				     &vk_allocator, &vk_command_pool);

	VkCommandBufferAllocateInfo vk_command_buffer_allocate_info = { 0 };
	vk_command_buffer_allocate_info.sType =
		VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
	vk_command_buffer_allocate_info.pNext = NULL;
	...
	result = vkAllocateCommandBuffers(
		device, &vk_command_buffer_allocate_info, &vk_command_buffer);
	if (result != VK_SUCCESS) {
		perror("vkAllocateCommandBuffers()");
		return -1;
	}

	VkCommandBufferBeginInfo vk_command_buffer_begin_info = { 0 };
	vk_command_buffer_begin_info.sType =
		VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
	vk_command_buffer_begin_info.pNext = NULL;
	vk_command_buffer_begin_info.flags = 0;
	vk_command_buffer_begin_info.pInheritanceInfo = NULL;

	result = vkBeginCommandBuffer(vk_command_buffer,
	...
	result = vkBindBufferMemory(device, src_buffer, vk_device_memory,
				    device_size);
	if (result != VK_SUCCESS) {
		perror("vkBindBufferMemory()");
		return -1;
	...
	result = vkBindBufferMemory(device, dst_buffer, vk_device_memory,
				    device_size);
	if (result != VK_SUCCESS) {
		perror("vkBindBufferMemory()");
		return -1;
	...
	VkBufferCopy vk_buffer_copy = { 0 };
	vk_buffer_copy.srcOffset = 0;
	vk_buffer_copy.dstOffset = 0;
	vk_buffer_copy.size = buffer_create_info.size;
	...
	vkFreeCommandBuffers(device, vk_command_pool, 1, &vk_command_buffer);
	vkDestroyCommandPool(device, vk_command_pool, &vk_allocator);
